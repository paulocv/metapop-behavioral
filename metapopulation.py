...
# * Iteration over .nodes() => nodes_iter(data=True) or __iter__()    - Done
# * Acessing .nodes() attrs   =>  .node[]      - Done
# * Iteration over .neighbors  => .neighbors_iter()  - Done
# * Edge property via .edges[i, j]  => .edge[i][j]  - Done
# * Edge iteration via .edges()    =>  .edges_iter()  - Done
# * degree returns a dict instead of a list of tuples  - Done 2021-01-29
...
class Metapop(nx.Graph):
    ...
    def pop_size(self, ni):
        ...
        return self.node[ni]["size"]

    def set_pop_size(self, ni, pop_size):
        self.node[ni]["size"] = pop_size
    ...
    def total_pop_size(self):
        return sum(self.pop_size(ni) for ni in self)
    ...
    def add_num(self, ni, state, num_add):
        ...
        self.node[ni]["num_" + state] += num_add
        self.node[ni]["size"] += num_add
    ...
    def set_tochange(self, ni, from_state, to_state, num):
        ...
        self.node[ni]["tochange_{}_{}".format(from_state, to_state)] = num

    def tochange(self, ni, from_state, to_state):
        return self.node[ni]["tochange_{}_{}".format(from_state, to_state)]
    ...
    def set_tomove(self, from_ni, state, nums_list):
        ...
        """
        self.node[from_ni]["tomove_{}".format(state)] = nums_list

    def tomove(self, from_ni, state):
        return self.node[from_ni]["tomove_{}".format(state)]
    ...
    def apply_moves(self, state):
        ...
        """
        for ni in self:
            for nj, num in zip(self.neighbors_iter(ni), self.tomove(ni, state)):
    ...
    def weight(self, ni, nj):
        return self.edge[ni][nj]["weight"]

    def set_weight(self, ni, nj, weight):
        self.edge[ni][nj]["weight"] = weight
        # self.edge[nj][ni]["weight"] = weight
    ...
    def make_travel_arrays(self, dtype=float):
        ...
        """
        for ni in self:
            trav = np.fromiter((self.weight(ni, nj) for nj in self.neighbors_iter(ni)),
                               dtype=dtype)
            self.node[ni]["travel_array"] = trav

    def travel_array(self, ni):
        """"""
        return self.node[ni]["travel_array"]
...
def stationary_population_from_degree(g, total_size):
    ...
    pop_dict = dict()
    # for ni, k in nx.degree(g):  # nx2
    for ni, k in nx.degree(g).items():  # nx1
...
def def_weight_by_pop_size(g, coeff, make_arrays=True, weight_key="weight"):
    ...
    for ni, nj in g.edges_iter():
        g.edge[ni][nj][weight_key] = coeff * g.pop_size(ni) * g.pop_size(nj) / total_size
