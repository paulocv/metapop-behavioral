...
# * Iteration over .neighbors  => .neighbors_iter()  - Not found
# * Edge property via .edges[i, j]  => .edge[i][j]  - Not found
# * Edge iteration via .edges()    =>  .edges_iter()  - Not found
...
class MetapopModel(object):
    ...
    def gen_calc_pinf_socialdist(self, beta, infective_state="I",
        ...
        if local_rho0_key is None:
            ...
            def local_rho0(ni):
                return self.g.node[ni][local_rho0_key]
    ...
    def epidemic_step_socialdist(self, reac_exponent=1, long_term_coef=1,
                                 globality_coef=1, detec_ratio=1, local_rho0_key=None):
        pass

    def epidemic_step_activation_local(self, act_thres,
                                       reac_exponent=1, long_term_coef=1,
                                       globality_coef=1,
                                       act_long_term_coef=1, detec_ratio=1,
                                       ):
        pass
    ...
    def step_socialdist(self, reac_exponent=1, long_term_coef=1,
                        globality_coef=1, detec_ratio=1,
                        travel_fac=1, local_rho0_key=None):
        """"""
        # Calculates and applies epidemic transitions
        self.epidemic_step_socialdist(reac_exponent=reac_exponent,
    ...
    def step_activation_local(self, act_thres,
                              reac_exponent=1, long_term_coef=1,
                              globality_coef=1, detec_ratio=1,
                              act_long_term_coef=1, travel_fac=1.):
        # Calculates and applies epidemic transitions
        self.epidemic_step_activation_local(act_thres=act_thres,
                                            reac_exponent=reac_exponent,
                                            long_term_coef=long_term_coef,
                                            globality_coef=globality_coef,
                                            detec_ratio=detec_ratio,
                                            act_long_term_coef=act_long_term_coef)
        for s1, s2 in self.transitions:
            self.g.apply_state_changes(s1, s2)
    ...
    def simulate_basic(self, tmax, init_mode=None, init_data=None,
        ...
        if init_node_attrs is not None:
            for ni in self.g:
                self.g.node[ni].update(init_node_attrs)
        ...
        current_statecount = {state: [self.g.num(ni, state) for ni in self.g]
                              for state in self.statelist}
        ...
        for state in self.statelist:
            statecount[state][0] = [self.g.num(ni, state) for ni in self.g]
            ...
            for state in self.statelist:
                ...
                if get_a_array:
                    for i, ni in enumerate(self.g):
                        a_array[i_0 + i_t][i] = self.g.node[ni][REAC_KEY]
                    # a_array[i_0 + i_t] = (self.g.node[ni][REAC_KEY] for ni in self.g)
    ...
    def simulate_activation_global(self, tmax, act_thres,
        ...
        sample_node_data = self.g.node[sample_node]
    ...
    def simulate_condit_global(self, tmax, init_mode=None, init_data=None,
        ...
        for i_t, t in enumerate(np.arange(tmax)):  # This allows for non-integer t
            ...
            # ---------------------------------
            # Event handling - activation/deactivation
            glob_num_next = self.g.total_num_in_statelist(monitor_states)
